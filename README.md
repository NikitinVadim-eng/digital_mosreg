...
2. Укажите **количество карточек** (по умолчанию **1**; `0` — весь каталог).
3. Нажмите **«Загрузить данные»** (кнопка блокируется на время загрузки).
4. Дождитесь прогресса — в таблице появятся строки.
5. При необходимости нажмите **«Скачать данные (XLSX)»**.

По умолчанию в форме — **одна** карточка (быстрый тест). Лимит из формы имеет приоритет над `DIGITAL_MOSREG_MAX_ITEMS` в compose на время запуска.
